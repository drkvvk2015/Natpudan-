...
import EnhancedMedicalHistory, {
  type MedicalHistoryItem,
  type SmokingHistory,
} from "../components/EnhancedMedicalHistory";
...
const COMMON_LAB_TESTS = [
  {
    test: "Hemoglobin",
    normalRange: "12-16 g/dL (F), 14-18 g/dL (M)",
    units: "g/dL",
  },
  {
...
export default function ClinicalCaseSheet() {
  ...
  const [selectedPatientRisk, setSelectedPatientRisk] = useState<
  ...
  // AI Diagnosis state
  const [liveDiagnosis, setLiveDiagnosis] =
    useState<LiveDiagnosisResponse | null>(null);
  ...
  const toggleClinicalFinding = (
    system: string,
    finding: string,
    normal: boolean = true
  ) => {
    const existingIndex = (clinicalFindings || []).findIndex(
      (f) => f.system === system && f.finding === finding
    );
  ...
  const handleReportUpload = async (
    ...
    for (let i = 0; i < (files || []).length; i++) {
      ...
      const newReport: UploadedReport = {
        ...
        name: file.name,
        type: file.name.toLowerCase().includes("lab")
          ? "lab"
          : file.name.toLowerCase().includes("rad") ||
            file.name.toLowerCase().includes("xray") ||
            file.name.toLowerCase().includes("ct") ||
            file.name.toLowerCase().includes("mri")
          ? "radiology"
          : file.name.toLowerCase().includes("path") ||
            file.name.toLowerCase().includes("biopsy")
          ? "pathology"
          : "other",
        date: new Date().toISOString().split("T")[0],
  ...
  const removeReport = (id: string) => {
    setUploadedReports(
      (uploadedReports || []).filter((report) => report.id !== id)
    );
  };
  ...
  const fetchLiveDiagnosis = useCallback(async () => {
    ...
    try {
      ...
      if (vitalSigns.oxygenSaturation)
        vitalSignsObj.SPO2 = vitalSigns.oxygenSaturation;
      if (vitalSigns.respiratoryRate)
        vitalSignsObj.RR = vitalSigns.respiratoryRate;
      ...
      const fullHistory = [
        ...
        presentHistory.chronology?.length &&
          `Timeline: ${presentHistory.chronology.join(" -> ")}`,
        enhancedHistory.medicalHistory?.length &&
          `Past Medical History: ${enhancedHistory.medicalHistory
            .map((h) => `${h.condition} (${h.diagnosedYear})`)
            .join(", ")}`,
        enhancedHistory.smokingHistory?.isSmoker &&
          `Smoking: ${enhancedHistory.smokingHistory.packsPerDay} packs/day for ${enhancedHistory.smokingHistory.yearsSmoked} years (${enhancedHistory.smokingHistory.packYears} pack-years)`,
        familyHistory?.length && `Family History: ${familyHistory.join(", ")}`,
        Object.values(reviewOfSystems).some((arr) => arr?.length) &&
          `Review of Systems: ${Object.entries(reviewOfSystems)
            .filter(([_, symptoms]) => symptoms?.length)
            .map(([system, symptoms]) => `${system}: ${symptoms.join(", ")}`)
            .join("; ")}`,
      ]
        .filter(Boolean)
        .join("\n\n");
  ...
  const handleSuggestTreatment = async () => {
    ...
    try {
      ...
      const response = await axios.post("/api/treatment-plans/suggest", {
        ...
        patient_gender: sex,
        comorbidities: (enhancedHistory.medicalHistory || []).map(
          (h) => h.condition
        ),
      });
  ...
  const handleGeneratePrescription = async () => {
    ...
    try {
      ...
      const response = await axios.post("/api/prescription/generate", {
        ...
        allergies: (enhancedHistory.medicalHistory || [])
          .filter((h) => h.condition?.toLowerCase().includes("allerg"))
          .map((h) => h.condition),
      });
    ...
    try {
      const reportData: DiagnosisReportData = {
        ...
        differential_diagnoses: (
          liveDiagnosis.differential_diagnoses || []
        ).map((d) => ({
        ...
        primary_diagnosis:
          liveDiagnosis.differential_diagnoses[0]?.diagnosis ||
          liveDiagnosis.differential_diagnoses[0]?.disease_name,
      };

      const blob = await generateDiagnosisReport(reportData);
      downloadPDF(
        blob,
        `clinical_case_sheet_${firstName || "patient"}_${
  ...
  const handleExportOPDCaseSheet = async () => {
    try {
      ...
      // Build OPDCaseSheetRequest in snake_case format to match backend Pydantic models
      const caseSheetData = {
        patient: {
          id: uhid || "N/A",
        ...
        medical_history: (enhancedHistory.medicalHistory || []).map(
          (item: any) => ({
        ...
        personal_history: {
          smoking: {
            status: enhancedHistory.smokingHistory?.isSmoker
              ? "Current"
              : "Never",
            packs_per_day: enhancedHistory.smokingHistory?.packsPerDay || 0,
            years: enhancedHistory.smokingHistory?.yearsSmoked || 0,
            quit_date: null,
            smoking_index: enhancedHistory.smokingHistory?.packYears || 0,
          },
          alcohol: {
            status: "Never",
            units_per_week: 0,
        ...
        clinical_examination: {
          vital_signs: {
            ...
            heart_rate: parseInt(vitalSigns.pulse) || 0,
            temperature: parseFloat(vitalSigns.temperature) || 0,
            respiratory_rate: parseInt(vitalSigns.respiratoryRate) || 0,
            oxygen_saturation: parseInt(vitalSigns.oxygenSaturation) || 0,
            weight: parseFloat(vitalSigns.weight) || 0,
            height: parseFloat(vitalSigns.height) || 0,
            bmi: parseFloat(vitalSigns.bmi) || 0,
          },
          general_examination: {
            consciousness: "Alert",
            pallor: false,
        ...
        diagnosis: {
          ...
          history_of_present_illness:
            presentHistory.chronology?.join(" -> ") || "",
          primary_diagnosis:
            liveDiagnosis?.differential_diagnoses?.[0]?.diagnosis ||
            assessment ||
            "Pending",
          secondary_diagnosis:
            liveDiagnosis?.differential_diagnoses
              ?.slice(1)
              .map((d: any) => d.diagnosis || d.disease_name) || [],
          icd_codes:
            liveDiagnosis?.differential_diagnoses
              ?.map((d: any) => d.icd_code)
              .filter(Boolean) || [],
          confidence: "Provisional",
        },
        treatment_plan: {
          medications: [],
          advice: [plan].filter((p) => p?.trim()),
          follow_up: {
        ...
        doctor_notes: assessment,
        doctor_name: "Dr. " + (firstName || "Doctor"),
        hospital_name: "Natpudan Medical Center",
      };

      // Send to backend
      const response = await axios.post(
        "/api/reports/opd-case-sheet",
        caseSheetData,
        {
          responseType: "blob",
          timeout: 60000,
      ...
      const fileName = `OPD_CaseSheet_${firstName || "patient"}_${
        new Date().toISOString().split("T")[0]
      }.pdf`;
      ...
      alert(`Failed to generate case sheet: ${errorMsg}`);
    } finally {
      setGeneratingReport(false);
    }
  };
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Comprehensive medical assessment with pre-defined options and easy-click
        interface
      </Typography>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={7}>
          <Paper
            elevation={3}
            sx={{ p: 3, maxHeight: "85vh", overflow: "auto" }}
          >
            <Box
              sx={{
                display: "flex",
            ...
            {selectedPatientId && (
              <Alert
                ...
                action={
                  selectedPatientRisk && (
                    <RiskBadge level={selectedPatientRisk} size="small" />
                  )
                }
              >
            ...
            <Accordion defaultExpanded sx={{ mb: 2 }}>
              ...
              <AccordionDetails>
                ...
                <Box sx={{ mb: 2 }}>
                  {Object.entries(COMPLAINT_TEMPLATES).map(
                    ([category, templates]) => (
                        ...
                        >
                          {templates.map((template) => (
                            <Chip
                              ...
                              size="small"
                              onClick={() => {
                                const lastComplaint = (complaints || [])[
                                  Math.max(0, (complaints || []).length - 1)
                                ];
                                if (lastComplaint && !lastComplaint.complaint) {
                                  ...
                                  if (
                                    !(presentHistory.chronology || []).includes(
                                      historyEntry
                                    )
                                  ) {
                                    setPresentHistory((prev) => ({
                                      ...
                                      onset:
                                        presentHistory.onset || "Recent onset",
                                    }));
                                  }
                                } else {
                                  addComplaint();
                    ...
                    >
                      {[
                        ...
                        <Chip
                          ...
                          size="small"
                          variant={
                            presentHistory.associatedSymptoms.includes(symptom)
                              ? "filled"
                              : "outlined"
                          }
                          onClick={() => {
                            setPresentHistory((prev) => ({
                              ...prev,
                              associatedSymptoms:
                                prev.associatedSymptoms.includes(symptom)
                                  ? prev.associatedSymptoms.filter(
                                      (s) => s !== symptom
                                    )
                    ...
                    >
                      {[
                        ...
                        <Chip
                          ...
                          size="small"
                          variant={
                            presentHistory.relievingFactors.includes(factor)
                              ? "filled"
                              : "outlined"
                          }
                          onClick={() => {
                            setPresentHistory((prev) => ({
                              ...prev,
                              relievingFactors: prev.relievingFactors.includes(
                                factor
                              )
                                ? prev.relievingFactors.filter(
                                    (f) => f !== factor
                                  )
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                      {[
                        ...
                        <Chip
                          ...
                          size="small"
                          variant={
                            presentHistory.aggravatingFactors.includes(factor)
                              ? "filled"
                              : "outlined"
                          }
                          onClick={() => {
                            setPresentHistory((prev) => ({
                              ...prev,
                              aggravatingFactors:
                                prev.aggravatingFactors.includes(factor)
                                  ? prev.aggravatingFactors.filter(
                                      (f) => f !== factor
                                    )
            ...
            <Accordion sx={{ mb: 2 }}>
              ...
              <AccordionDetails>
                {Object.entries(EXAMINATION_SYSTEMS).map(
                  ([system, { icon, findings }]) => (
                    <Card key={system} variant="outlined" sx={{ mb: 2 }}>
                      <CardContent>
                        ...
                        >
                          Left-click: Normal (green) • Right-click: Abnormal
                          (red)
                        </Typography>
                        <Box
                        ...
                        >
                          {findings.map((finding) => {
                            const existing = clinicalFindings.find(
                              (f) =>
                                f.system === system && f.finding === finding
                            );
                            ...
                            return (
                              <Chip
                                ...
                                onClick={() =>
                                  toggleClinicalFinding(system, finding, true)
                                }
                                onContextMenu={(e) => {
                                  e.preventDefault();
            ...
            <Accordion sx={{ mb: 2 }}>
              ...
              <AccordionDetails>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ mb: 2 }}>
                    {Object.entries(INVESTIGATION_CATEGORIES).map(
                      ([category, tests]) => (
                        <Card
                          key={category}
                          variant="outlined"
                          ...
                          >
                            {tests.map((test) => (
                              <Chip
                                ...
                                size="small"
                                onClick={() =>
                                  addInvestigationAdvice(
                                    category,
                                    [test],
                                    `${category} workup indicated`,
                                    "routine"
                                  )
                                }
                                sx={{ cursor: "pointer" }}
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={12} md={3}>
                          <TextField
                            ...
                            value={test.test}
                            onChange={(e) =>
                              updateLabTest(test.id, "test", e.target.value)
                            }
                            size="small"
                          />
                        </Grid>
                        <Grid item xs={6} md={2}>
                          <TextField
                            ...
                            value={test.result}
                            onChange={(e) =>
                              updateLabTest(test.id, "result", e.target.value)
                            }
                            size="small"
                          />
                        </Grid>
                        <Grid item xs={6} md={2}>
                          <TextField
                            ...
                            value={test.units}
                            onChange={(e) =>
                              updateLabTest(test.id, "units", e.target.value)
                            }
                            size="small"
                          />
                        </Grid>
            ...
            <Box sx={{ mb: 2, display: "flex", justifyContent: "center" }}>
              <Button
                ...
                disabled={
                  isAnalyzing ||
                  (complaints || []).filter((c) => c.complaint?.trim())
                    .length === 0
                }
                sx={{ px: 4, py: 1.5 }}
              >
            ...
            {liveDiagnosis && (
              <Box>
                ...
                <Box sx={{ mb: 2 }}>
                  ...
                  >
                    Data Completeness:{" "}
                    {Math.round((liveDiagnosis.data_completeness || 0) * 100)}%
                  </Typography>
                  <LinearProgress
                    variant="determinate"

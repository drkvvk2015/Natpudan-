# [EMOJI] FUTURISTIC KNOWLEDGE BASE FEATURES
...
**Algorithm:**
```
RRF(doc) =  /(k + vector_rank(doc)) + (1-)/(k + bm25_rank(doc))
```
Where:
- `k` = 60 (constant)
- `` = weight (0=BM25 only, 1=vector only)
...
**Best Practices:**
- Use `=0.7` for medical documents (favor semantic understanding)
- Use `=0.3` for code/drug names (favor exact keyword matching)
- Use `=0.5` for general queries
...
[OK] Hybrid Search Engine (328 lines)
[OK] RAG Service (348 lines)  
[OK] Medical Entity Extractor (350 lines)
[OK] PubMed Integration (438 lines)
[OK] Knowledge Graph (485 lines)
[OK] 8 New API Endpoints
[OK] Dependencies Installed (rank-bm25)

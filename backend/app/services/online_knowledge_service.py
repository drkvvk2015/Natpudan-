...
class OnlineKnowledgeService:
    ...
    async def search_pubmed(
        ...
        """
        cache_key = f"pubmed_{hashlib.sha256(query.encode()).hexdigest()[:32]}_{max_results}"
    ...
    async def get_clinical_guidelines(
        ...
        """
        cache_key = f"guidelines_{hashlib.sha256(condition.encode()).hexdigest()[:32]}"
    ...
    async def get_drug_information(
        ...
        """
        cache_key = f"drug_{hashlib.sha256(drug_name.encode()).hexdigest()[:32]}"
    ...
    async def get_medical_news(
        ...
        """
        cache_key = f"news_{hashlib.sha256(topic.encode()).hexdigest()[:32]}_{max_results}"

...
# Tail job output with error handling
try {
    while ($true) {
        $jobs = Get-Job -Name backend, frontend -ErrorAction SilentlyContinue
        if (-not $jobs -or $jobs.Count -eq 0) {
            Write-Warning "No active jobs found. Exiting..."
            break
        }
        
        $jobs | Receive-Job -Keep -ErrorAction SilentlyContinue | ForEach-Object { 
            if ($_ -and $_ -ne "") {
                Write-Host $_
            }
        }
        ...
        if (-not $runningJobs) {
            Write-Warning "All jobs have stopped. Exiting..."
            break
        }
